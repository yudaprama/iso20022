...
type RequestToModifyPayment struct {
	...
	//
	Modification *model.RequestedModification `xml:"Mod"`
}

func (r *RequestToModifyPayment) AddAssignment() *model.CaseAssignment {
	r.Assignment = new(model.CaseAssignment)
	return r.Assignment
}

func (r *RequestToModifyPayment) AddCase() *model.Case {
	r.Case = new(model.Case)
	return r.Case
}

func (r *RequestToModifyPayment) AddUnderlying() *model.PaymentInstructionExtract {
	r.Underlying = new(model.PaymentInstructionExtract)
	return r.Underlying
}

func (r *RequestToModifyPayment) AddModification() *model.RequestedModification {
	r.Modification = new(model.RequestedModification)
